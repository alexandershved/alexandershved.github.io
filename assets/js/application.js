...
/***/ function(module, exports, __webpack_require__) {
	...
	var __svg__ = { filename: "/assets/svg/1500497486623.icons.svg" };
	...
	exports.default = function (list) {
	  ...
	  updateValue = function updateValue(val, ttl, fld, clr) {
	    ...
	    if (fld) {
	      ...
	      label = document.createElement('div');
	      label.className = 'list__label';
	      label.textContent = window.might.stat.columns_name[fld] || fld;
	      list.children[0]._insertBefore(label);
	    }
...
/***/ function(module, exports, __webpack_require__) {
	...
	exports.default = function () {
	    ...
	    if (filterControl) {
	      (function () {
	        ...
	        statParams.filters_stock.forEach(function (filter) {
	          ...
	          var withCondition = settings.filter.filter(function (el) {
	            return el[1] === filter.field;
	          })[0][2] === 'condition';
	          ...
	          if (!withCondition) {
	            ...
	            filterEl.className = 'condition js-condition is-margin-top';
	            filterEl.innerHTML = '\n            <div class="list js-list">\n              <div class="list__wrap">\n                <div class="list__value js-list-value"></div>\n                <div class="list__dropdown">\n                  <div class="list__items js-list-items">\n                    <div class="list__item js-list-item" data-value="more">more</div>\n                    <div class="list__item js-list-item" data-value="more,equally">more or equally</div>\n                    <div class="list__item js-list-item" data-value="less">less</div>\n                    <div class="list__item js-list-item" data-value="less,equally">less or equally</div>\n                    <div class="list__item js-list-item" data-value="equally">equally</div>\n                    <div class="list__item js-list-item" data-value="interval">interval</div>\n                  </div>\n                </div>\n              </div>\n            </div>\n            <input class="condition__input js-condition-val-one" />\n            <span class="condition__to js-condition-to">to</span>\n            <input class="condition__input js-condition-val-two" />';

	            filterControl.appendChild(filterEl);
	            (0, _conditionEvent2.default)(filterEl);
	            filterEl.updateValue(filter.value || [0], (filter.condition || ['more']).join(','), filter.field, true);
	          }

	          filterEl.addEventListener('change:list', function (event) {
	            if (filterEl.condition) {
	              (0, _update2.default)({ filter_stock: { field: filter.field, value: filterEl.value, condition: filterEl.condition, title: filter.field } });
	            } else {
	  ...
	  function useSettings() {
	    ...
	    if (Array.isArray(result.filter) && cohortFilterAdd) {
	      (function () {
	        var itemsWrap = void 0;
	        itemsWrap = cohortFilterAdd.querySelector('.js-list-items');
	        itemsWrap.innerHTML = '';

	        result.filter.forEach(function (filter) {
	          if (Array.isArray(filter) && filter.length >= 2) {
	            var item = document.createElement('div');
	            item.className = 'list__item js-list-item';
	            item.dataset.value = filter[1];
	            item.textContent = filter[0];
	...
	var _conditionEvent = __webpack_require__(67);

	var _conditionEvent2 = _interopRequireDefault(_conditionEvent);
...
/***/ function(module, exports, __webpack_require__) {
	...
	exports.default = function (condition) {
	  var list = condition.querySelector('.js-list');
	  var inputOne = condition.querySelector('.js-condition-val-one');
	  var inputTwo = condition.querySelector('.js-condition-val-two');
	  var to = condition.querySelector('.js-condition-to');

	  var updateValue = void 0;
	  var setValue = void 0;

	  updateValue = function updateValue(val, cond, fld, clr) {
	    var isUpdate = false;
	    var _cond = cond || list.value || 'more';

	    if (fld) {
	      var f = window.might.stat.columns_name[fld] || fld;
	      list.updateValue(null, f);
	    }

	    if (_cond !== condition.condition) {
	      isUpdate = true;
	      list.updateValue(_cond);
	    ...
	    if (!val) {
	      ...
	      if (val[0] !== condition.value[0]) {
	        isUpdate = true;
	      }

	      inputOne.value = val[0] || 0;
	      condition.value[0] = Number(val[0]);

	      if (condition.condition === 'interval') {
	        if (val[0] > val[1]) {
	          val[1] = val[0];
	        }

	        if (val[1] !== condition.value[1]) {
	          isUpdate = true;
	        }

	        inputTwo.value = val[1];

	        if (Number(val[0]) > Number(val[1])) {
	          condition.value[1] = Number(val[0]);
	        } else {
	          condition.value[1] = Number(val[1]);
	        }
	      }
	    }

	    if (condition.condition === 'interval') {
	      inputTwo.style.display = '';
	      to.style.display = '';

	      if (!condition.value[1] && condition.value[1] !== 0) {
	        inputTwo.value = condition.value[0];
	        condition.value[1] = Number(inputTwo.value);
	      }
	    } else {
	    ...
	    if (clr) {
	      var clearBtn = document.createElement('div');
	      clearBtn.className = 'list__clear';
	      condition.appendChild(clearBtn);
	      clearBtn.addEventListener('click', function () {
	        setValue();
	      });
	    }
	  ...
	  setValue = function setValue(value, cond) {
	    if (updateValue(value, cond)) {
	      condition.triggerEvent('change:list');
	    }
	  };

	  condition.value = [0];

	  if (list && inputOne && inputTwo) {
	    (0, _listEvent2.default)(list);
	    list.updateValue('more');
	    inputTwo.style.display = 'none';
	    to.style.display = 'none';
	  } else {
	  ...
	  list.addEventListener('change', function () {
	    setValue(condition.value, list.value);
	  });
	  ...
	  inputOne.addEventListener('change', function () {
	    ...
	    var value = [Number(inputOne.value)];

	    if (list.value === 'interval') {
	      value.push(Number(inputTwo.value));
	    }
	  ...
	  inputTwo.addEventListener('change', function () {
	    ...
	    setValue([Number(inputOne.value), Number(inputTwo.value)]);
	  });

	  condition.updateValue = updateValue;
	  condition.setValue = setValue;
	};
	                  ...
	                  if (pos !== -1) {
	                    if (!f.value || !f.title) {
	                      ...
	                      if (f.hasOwnProperty('condition')) {
	                        params.filters_stock[pos].condition = f.condition.split(',');
	                      }
	                    }
	                    isUpdateStock = true;
	                    isUpdate = true;
	...
	exports.default = function () {
	  ...
	  var setVariables = function setVariables(values) {
	    ...
	    values.forEach(function (val, index) {
	      window.might.stat.columns_name['p' + (index + 1)] = val.name;
	    ...
	    if (/^p\d{1,2}$/.test(window.might.stat.params.segments[0])) {
	      segmentVariables.setValue(window.might.stat.params.segments[0]);
	    }
	  ...
	  if (isCampaign) {
	    ...
	    if (__trafficSourceVal__ !== trafficSourceVal) {
	      ...
	      if (trafficSourceVal || trafficSourceVal === 0) {
...
/***/ function(module, exports, __webpack_require__) {
	...
	exports.default = function (data, hasEdit) {
	  ...
	  var formSamplePathOn = popupBody.querySelector('.js-form-sample-path-on');
	  var formSamplePathOff = popupBody.querySelector('.js-form-sample-path-off');
	  var formCostLabel = popupBody.querySelector('.js-form-cost-label');
	  var formDomainPathAuto = popupBody.querySelector('.js-form-domain-path-auto');
	  var formDomainPathManual = popupBody.querySelector('.js-form-domain-path-manual');
	  var formDomainPathManualData = popupBody.querySelector('.js-form-domain-path-manual-data');
	  var formDomainPathManualDataHttp = popupBody.querySelector('.js-form-domain-path-manual-data-http');
	  ...
	  if (String(data.domain)) {
	    ...
	    (0, _campaignAdd.updateDomainPath)(formDomain, formDomainPathManualDataHttp);
	  }
	  ...
	  if (data.path_type) {
	    switch (data.path_type) {
	      case 'auto':
	        formDomainPathAuto.classList.add('is-select');
	        formDomainPathManual.classList.remove('is-select');
	        formDomainPathManualData.parentNode.style.display = 'none';
	        break;
	      case 'manual':
	        formDomainPathManual.classList.add('is-select');
	        formDomainPathAuto.classList.remove('is-select');
	        formDomainPathManualData.parentNode.style.display = 'block';
	        break;
	      default:
	        formDomainPathAuto.classList.add('is-select');
	        formDomainPathManual.classList.remove('is-select');
	  ...
	  formDomainPathManualData.value = data.path_url;
	  ...
	  switch (data.beta_distribution) {
	    case '1':
	      ...
	      (0, _campaignAdd.updateAllWeight)(popupBody, 'none');
	      break;
	    case '0':
	      formBetaDistributionOff.classList.add('is-select');
	      formBetaDistributionOn.classList.remove('is-select');
	      (0, _campaignAdd.updateAllWeight)(popupBody, 'table-cell');
	      break;
	    default:
	      formBetaDistributionOff.classList.add('is-select');
	      formBetaDistributionOn.classList.remove('is-select');
	  ...
	  switch (data.same_path) {
	    case '1':
	      formSamplePathOn.classList.add('is-select');
	      formSamplePathOff.classList.remove('is-select');
	      break;
	    case '0':
	      formSamplePathOff.classList.add('is-select');
	      formSamplePathOn.classList.remove('is-select');
	      break;
	    default:
	      formSamplePathOff.classList.add('is-select');
	      formSamplePathOn.classList.remove('is-select');
	  ...
	  if (Array.isArray(data.double_postback)) {
	    ...
	    for (var i = 1; i < data.double_postback.length; i++) {
	      var addBtns = popupBody.querySelectorAll('.js-form-postback-url-add');
	      addBtns[addBtns.length - 1].triggerEvent('click');

	      var inputs = popupBody.querySelectorAll('.js-form-postback-url');
	      inputs[inputs.length - 1].value = data.double_postback[i] || '';
	    }
...
/***/ function(module, exports, __webpack_require__) {
	...
	exports.updateAllWeight = updateAllWeight;
	exports.updateDomainPath = updateDomainPath;
	...
	exports.default = function () {
	    ...
	    (0, _fetchApi.fetchData)('/sites/list', {
	      ...
	      for (var i in obj) {
	        if (obj.hasOwnProperty(i)) {
	          ...
	          domainsValues[item.id] = item;
	        }
	      }
	      updateDomainPath(formDomain, formDomainPathManualDataHttp);
	  ...
	  formDomain.addEventListener('change', function () {
	    updateDomainPath(formDomain, formDomainPathManualDataHttp);
	  });
	  ...
	  formBetaDistributionOn.addEventListener('click', function () {
	    formBetaDistributionOn.classList.add('is-select');
	    formBetaDistributionOff.classList.remove('is-select');
	    updateAllWeight(popupBody, 'none');
	  });

	  formBetaDistributionOff.addEventListener('click', function () {
	    formBetaDistributionOff.classList.add('is-select');
	    formBetaDistributionOn.classList.remove('is-select');
	    updateAllWeight(popupBody, 'table-cell');
	  });

	  formDomainPathAuto.addEventListener('click', function () {
	    formDomainPathAuto.classList.add('is-select');
	    formDomainPathManual.classList.remove('is-select');
	    formDomainPathManualData.parentNode.style.display = 'none';
	  });

	  formDomainPathManual.addEventListener('click', function () {
	    formDomainPathManual.classList.add('is-select');
	    formDomainPathAuto.classList.remove('is-select');
	    formDomainPathManualData.parentNode.style.display = 'block';
	  });

	  formSamplePathOn.addEventListener('click', function () {
	    formSamplePathOn.classList.add('is-select');
	    formSamplePathOff.classList.remove('is-select');
	  });

	  formSamplePathOff.addEventListener('click', function () {
	    formSamplePathOff.classList.add('is-select');
	    formSamplePathOn.classList.remove('is-select');
	  });
	  ...
	  function saveCampaign(isClose) {
	    ...
	    if (formSamplePathOn.classList.contains('is-select')) {
	      samplePath = '1';
	    }

	    var domainPathManualData = false;
	    var domainPathType = 'auto';
	    if (formDomainPathManual.classList.contains('is-select')) {
	      domainPathManualData = formDomainPathManualData.value.trim();
	      domainPathType = 'manual';
	    }
	    ...
	    if (doublePostback.length === 1) {
	      doublePostback = doublePostback[0];
	    }
	    ...
	    var data = {
	      ...
	      same_path: samplePath,
	      domain_path: domainPathManualData,
	      domain_path_type: domainPathType
	    };
	...
	function updateAllWeight(popupBody, style) {
	  var jsPathOfferWeight = popupBody.getElementsByClassName('js-path-offer-weight');
	  var jsPathLanderWeight = popupBody.getElementsByClassName('js-path-lander-weight');
	  var jsPathWeight = popupBody.getElementsByClassName('js-path-weight');
	  var jsPathLanderLabelWeight = popupBody.getElementsByClassName('js-path-lander-label-weight');
	  var jsPathOfferLabelWeight = popupBody.getElementsByClassName('js-path-offer-label-weight');
	  var jsPathLabelWeight = popupBody.getElementsByClassName('js-path-label-weight');

	  var updateWeightStyle = function updateWeightStyle(element, label, val) {
	    if (element) {
	      for (var elem in element) {
	        if (element[elem].localName && element[elem].localName === 'input') {
	          element[elem].parentNode.parentNode.style.display = val;
	        }
	      }
	    }

	    if (label) {
	      for (var _elem in label) {
	        if (label[_elem].localName && label[_elem].localName === 'div') {
	          label[_elem].parentNode.style.display = val;
	        }
	      }
	    }
	  };

	  updateWeightStyle(jsPathOfferWeight, jsPathOfferLabelWeight, style);
	  updateWeightStyle(jsPathLanderWeight, jsPathLanderLabelWeight, style);
	  updateWeightStyle(jsPathWeight, jsPathLabelWeight, style);
	}

	var domainsValues = [];

	function updateDomainPath(formDomain, formDomainPathManualDataHttp) {
	  if (domainsValues[formDomain.value]) {
	    if (domainsValues[formDomain.value].is_owner && domainsValues[formDomain.value].is_owner === true) {
	      formDomainPathManualDataHttp.value = 'http://' + domainsValues[formDomain.value].name + '/';
	    } else {
	      formDomainPathManualDataHttp.value = 'http://' + window.__settings__.user_hash + '.' + domainsValues[formDomain.value].name + '/';
	    }
	  } else {
	    formDomainPathManualDataHttp.value = '';
	  }
	}
	...
	exports.default = {
	  render: function render(response) {
	    ...
	    if ((typeof sum === 'undefined' ? 'undefined' : _typeof(sum)) === 'object' && (typeof values === 'undefined' ? 'undefined' : _typeof(values)) === 'object') {
	      ...
	      medianOne = Math.ceil(medianOne, -2);
	      medianTwo = Math.ceil(medianTwo, -2);
